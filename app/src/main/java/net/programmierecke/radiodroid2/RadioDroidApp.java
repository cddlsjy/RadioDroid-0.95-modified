...
public class RadioDroidApp extends MultiDexApplication {
    ...
    private void importBuiltInStations() {
        ...
        boolean hasImported = prefs.getBoolean("has_imported_builtin_stations", false);
        ...
        if (!hasImported) {
            try {
                ...
                // 标记已导入
                prefs.edit().putBoolean("has_imported_builtin_stations", true).apply();
                Log.i("RadioDroidApp", "Built-in stations imported successfully");
            } catch (Exception e) {
                Log.e("RadioDroidApp", "Failed to import built-in stations: " + e.getMessage());
            }
        }
    }
